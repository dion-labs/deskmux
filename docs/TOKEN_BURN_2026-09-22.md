...
## Checkpoint 27 — v0.2.6 site completion and next log scope

Read independent site receipt: exact publication/CI/source/ZIP5,219,263/hash confirmed; local0.2.6 version/schema/archive/notes/copy complete. Build138ms,1440/390/320 affectedtext/link checks and3actualHEAD200 PASS. Prior63 browser matrix retained with unchanged behavior hashes. No deploy/mainpush/install. v0.2.6 release/link workflow complete within authorized local site scope.

User/root directs bounded persisted network/handoff log acceptance. Inspected actual writers in DeskMuxService/main.swift recordNetwork and BackgroundAgentModel.swift recordHandoff: synchronous JSONL, timestamp+event+peer+build+detail; handoff adds anchor/relaystate/error/Flowconflict. Both open/seek/writeJSON/writeNewline per record and swallow errors; no rotation policy. Production service/model constructors start native state, so must not instantiate them. Plan behavior-preserving extraction into explicit-directory DeskMuxCore writer, with productioncallers retaining existing path/schema/timing. Test actual files in tempdirs: recreation/order/concurrentappend, external rename/delete rotation, blockedpath recovery, schema/privacycanaries. No automaticrotation invented. Freeform errors/details are currently verbatim diagnostic metadata and cannot be called fully redacted; fixtures will identify precise privacyboundary.

## Checkpoint 28 — unchanged persisted writers reproduce corrupt concurrent records

Behavior-preserving DeskMuxCore extraction compiled. Six test functions each cover bothnetwork/handoff; five passed persistence/recreation/order, externalrename/delete rotation, blocked-directory/log-path recovery and JSON schema/escaping. Concurrent8writers x40records with8KiB synthetic Unicode/newline detail FAILED for both streams: JSON parser garbage-at-end (columns8418handoff/8309network), /tmp/deskmux-burn-runtime-log-baseline.log. Thus actualfiles, not mocked FileHandles, expose seek/write/create races.

Candidate fix uses atomic O_CREAT|O_APPEND open, flock per inode around one combinedJSON+newline FileHandle.write, and closes eachcall; no check-then-create/truncate or stale seekoffset. Existing synchronous nonthrowingcontract, schema, timestamps and productionpaths retained. Directory seam reads nothing itself beyond destination; no service/UI constructors used. Privacycanaries establish no forgedJSONrows or added top-level clipboard/key fields; freeform detail/error remain verbatim and are not claimed redacted. No actual private payload reaches these APIs in the fixtures; absence of plaintext in live services remains a separate gate. Scoped candidate sent for independentreview/releasecoordination; focusedfix run next.
